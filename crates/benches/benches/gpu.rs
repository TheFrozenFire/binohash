...
fn bench_gpu_digest_search_r2_nth(c: &mut Criterion) {
    use rand::SeedableRng;
    use rand_chacha::ChaCha8Rng;
    ...
    let config = script::QsbConfig::config_a();
    let pin_nonce = hors::NonceSig::derive("bench_nth_pin");
    let round1_nonce = hors::NonceSig::derive("bench_nth_r1");
    let round2_nonce = hors::NonceSig::derive("bench_nth_r2");
    ...
    c.bench_function("GPU digest Round 2 nth (n=150 t=9, 65536 subsets)", |b| {
        b.iter(|| {
            m.search_digest_batch_nth(
                black_box(&params),
                t as u32,
                n as u32,
                0,
...
fn bench_subset_preprocessing(c: &mut Criterion) {
    let n = 150_usize;
    let t = 9_usize;
    let batch_size: usize = 65536;

    c.bench_function(&format!("CPU nth_combination × {batch_size} (n=150 t=9)"), |b| {
        b.iter(|| {
            let subsets: Vec<u32> = (0..batch_size as u128)
                .flat_map(|idx| {
                    let combo = subset::nth_combination(n, t, idx).expect("valid");
...
criterion_group!(
    ...
    bench_gpu_digest_search_r2_nth,
    bench_subset_preprocessing,
